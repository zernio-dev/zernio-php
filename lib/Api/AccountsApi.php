...
class AccountsApi
{
    ...
    /**
     * Operation disconnectAdsWithHttpInfo
     *
     * Disconnect ads from an account
     *
     * @param  string $account_id The SocialAccount ID (parent posting account for same-token/separate-token platforms) (required)
    ...
    /**
     * Operation disconnectAdsAsyncWithHttpInfo
     *
     * Disconnect ads from an account
     *
     * @param  string $account_id The SocialAccount ID (parent posting account for same-token/separate-token platforms) (required)
    ...
    public function disconnectAdsRequest($account_id, $disconnect_ads_request, string $contentType = self::contentTypes['disconnectAds'][0])
    {
        ...
        if ($account_id === null || (is_array($account_id) && count($account_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $account_id when calling disconnectAds'
            );
        }
        ...
        // for model (json/xml)
        if (isset($disconnect_ads_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($disconnect_ads_request));
            } else {
                $httpBody = $disconnect_ads_request;
            }
