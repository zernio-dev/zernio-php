...
class InvitesApi
{
    ...
    {
        list($response) = $this->createInviteTokenWithHttpInfo($create_invite_token_request, $contentType);
        return $response;
    }
     ...
     */
    public function createInviteTokenAsync($create_invite_token_request, string $contentType = self::contentTypes['createInviteToken'][0])
    {
        return $this->createInviteTokenAsyncWithHttpInfo($create_invite_token_request, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    public function createInviteTokenRequest($create_invite_token_request, string $contentType = self::contentTypes['createInviteToken'][0])
    {

        // verify the required parameter 'create_invite_token_request' is set
        if ($create_invite_token_request === null || (is_array($create_invite_token_request) && count($create_invite_token_request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $create_invite_token_request when calling createInviteToken'
            );
        }


        $resourcePath = '/v1/invite/tokens';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($create_invite_token_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($create_invite_token_request));
            } else {
                $httpBody = $create_invite_token_request;
            }
