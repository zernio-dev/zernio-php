...
class GetAccountHealth200ResponsePlatformConnection implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    protected static $openAPITypes = [
        ...
        'inbound_webhook_subscribed' => 'bool'
    ];
    ...
    protected static array $openAPINullables = [
        ...
        'phone_status' => true,
        'meta_error' => false,
        'inbound_webhook_subscribed' => true
    ];
    ...
    protected static $setters = [
        ...
        'inbound_webhook_subscribed' => 'setInboundWebhookSubscribed'
    ];
    ...
    protected static $getters = [
        ...
        'inbound_webhook_subscribed' => 'getInboundWebhookSubscribed'
    ];
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('inbound_webhook_subscribed', $data ?? [], null);
    }
    ...
    */
    ...
    /**
     * Gets inbound_webhook_subscribed
     *
     * @return bool|null
     */
    public function getInboundWebhookSubscribed()
    {
        return $this->container['inbound_webhook_subscribed'];
    }

    /**
     * Sets inbound_webhook_subscribed
     *
     * @param bool|null $inbound_webhook_subscribed From the phone number's Meta health_status. `false` = Meta says Zernio is not subscribed to the message webhook for this number, so inbound messages are not delivered even though the number is CONNECTED and can still send. Fix by re-subscribing (reconnect the number); if it stays `false`, the number is routed to a different WhatsApp Business Account (typically after linking it to a Facebook Page). `true` = Meta reports no such problem. `null` = Meta did not report it (read failed or no health_status), not evidence either way.
     *
     * @return self
     */
    public function setInboundWebhookSubscribed($inbound_webhook_subscribed)
    {
        if (is_null($inbound_webhook_subscribed)) {
            array_push($this->openAPINullablesSetToNull, 'inbound_webhook_subscribed');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('inbound_webhook_subscribed', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        $this->container['inbound_webhook_subscribed'] = $inbound_webhook_subscribed;
