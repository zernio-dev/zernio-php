...
class GetInboxPostComments200ResponseCommentsInner implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     *
     * @param bool|null $replies_has_more Facebook only. True when replies[] (capped at 10) does not hold the comment's full reply thread; fetch the rest by passing the comment id as the `commentId` query parameter to GET /v1/inbox/comments/{postId} (or, for backwards compatibility, as `postId`). Absent (not false) on every other platform, including Instagram, which has no equivalent signal.
     *
     * @return self
     */
     ...
     * Sets is_hidden
     *
     * @param bool|null $is_hidden Whether the comment is currently hidden. On Instagram accounts connected through Facebook Login, Meta leaves hidden comments (and their replies) out of the list entirely, so they never appear with isHidden true; Instagram Login accounts and Facebook Pages return them with isHidden true.
     *
     * @return self
     */
