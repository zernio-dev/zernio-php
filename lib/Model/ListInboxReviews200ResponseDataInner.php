...
class ListInboxReviews200ResponseDataInner implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public const RECOMMENDATION_TYPE_POSITIVE = 'positive';
    public const RECOMMENDATION_TYPE_NEGATIVE = 'negative';
    ...
    {
        return [
            self::RECOMMENDATION_TYPE_POSITIVE,
            self::RECOMMENDATION_TYPE_NEGATIVE,
        ];
    }
    ...
    */
     ...
     *
     * @param string|null $recommendation_type Facebook recommendation: positive means recommends, negative means does not recommend. Null or absent when unavailable; absent for other platforms. Independent of the numeric rating.
     *
     * @return self
     */
    public function setRecommendationType($recommendation_type)
    {
        if (is_null($recommendation_type)) {
            array_push($this->openAPINullablesSetToNull, 'recommendation_type');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('recommendation_type', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        ...
        $allowedValues = $this->getRecommendationTypeAllowableValues();
        if (!is_null($recommendation_type) && !in_array($recommendation_type, $allowedValues, true)) {
