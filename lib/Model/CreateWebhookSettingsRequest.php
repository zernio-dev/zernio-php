...
class CreateWebhookSettingsRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets account_ids
     *
     * @param string[]|null $account_ids Connected accounts this subscription receives events for. Omit or send an empty array to receive every account. Every id must be an account in your team, otherwise the request fails with 404 `account_not_found` and nothing is created. Combine with `profileIds` to narrow further; both must match.
     *
     * @return self
     */
    public function setAccountIds($account_ids)
    {
        ...
        if ((count($account_ids) > 50)) {
            throw new \InvalidArgumentException('invalid value for $account_ids when calling CreateWebhookSettingsRequest., number of items must be less than or equal to 50.');
        }
