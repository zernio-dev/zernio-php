...
class CreateStandaloneAdRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public const AI_DISCLOSURE_OPT_IN = 'OPT_IN';
    public const AI_DISCLOSURE_OPT_OUT = 'OPT_OUT';
     ...
     * @return string[]
     */
    public function getAiDisclosureAllowableValues()
    {
        return [
            self::AI_DISCLOSURE_OPT_IN,
            self::AI_DISCLOSURE_OPT_OUT,
        ];
    }
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('ai_disclosure', $data ?? [], null);
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getAiDisclosureAllowableValues();
        if (!is_null($this->container['ai_disclosure']) && !in_array($this->container['ai_disclosure'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'ai_disclosure', must be one of '%s'",
                $this->container['ai_disclosure'],
                implode("', '", $allowedValues)
            );
        }
    ...
    public function setAiDisclosure($ai_disclosure)
    {
        ...
        $this->container['ai_disclosure'] = $ai_disclosure;
     ...
     * Sets budget_type
     *
     * @param string|null $budget_type Required on legacy, multi-creative and Performance Max shapes. Inherited on attach. OpenAI Ads creation accepts lifetime only; sending daily returns 422 (daily caps can be set afterwards with PUT /v1/ads/campaigns/{campaignId}). OpenAI Ads lifetime budgets require `endDate` to give the lifetime cap a spend window.
     *
     * @return self
     */
     ...
     *
     * @param string|null $campaign_status Meta, Google, and ChatGPT (OpenAI). Overrides `status` for the campaign level alone, so you can create a live campaign whose ad set and ad stay paused, or the reverse. Omitted, it follows `status`.
     *
     * @return self
     */
     ...
     *
     * @param string|null $existing_campaign_id Meta, Google Ads, LinkedIn and TikTok. On TikTok: creates the ad group and the ad under this existing campaign; the campaign is neither created nor activated and its objective must fit `goal`; not with `smartPlus`. On Meta: add the new ad set under this EXISTING campaign instead of creating a new one (multi-ad-set audience testing). The new ad set's budget is matched to the campaign's mode automatically: for a CBO campaign (campaign-level budget) omit `budgetAmount`/`budgetType`, since the campaign owns the budget; for an ABO campaign pass them (they go on the new ad set). On LinkedIn: create a new Campaign (and its Creative) under this EXISTING CampaignGroup. On Google Ads: create a new ad group under this EXISTING campaign; the new ad group inherits the campaign's budget, so omit `budgetAmount`/`budgetType` (and any bidding field), or the request returns 400. On failure only the entities we authored are cleaned up; the pre-existing parent is left untouched and is never (re)activated. Mutually exclusive with `adSetId` and `creatives[]`.
     *
     * @return self
     */
     ...
     * Sets country_groups
     *
     * @param string[]|null $country_groups Meta only. Continents and trade blocs (`geo_locations.country_groups`), for targeting a whole region without listing its countries. Combines with `countries` rather than replacing it. Discoverable via `GET /v1/ads/targeting/search?dimension=geo&geoType=country_group`.
     *
     * @return self
     */
     ...
     * Sets regions
     *
     * @param \Zernio\Model\CreateStandaloneAdRequestRegionsInner[]|null $regions Region-level (state/province) geo targeting (Meta, Google and TikTok). Each region is targeted by the platform's opaque `key` (the region ID) which can be looked up via `GET /v1/ads/targeting/search?dimension=geo&q=<name>&countryCode=<ISO>`. An entry may also be the key alone as a plain string.
     *
     * @return self
     */
     ...
     * Sets metros
     *
     * @param \Zernio\Model\CreateStandaloneAdRequestZipsInner[]|null $metros DMA / metro-area geo targeting (Meta and TikTok). `key` is the platform's metro ID from /v1/ads/targeting/search?dimension=geo&geoType=metro (TikTok metros appear as type `metro`, e.g. the New York DMA).
     *
     * @return self
     */
     ...
     * Sets start_date
     *
     * @param \DateTime|null $start_date Meta only. Ad-set start time (ISO 8601, e.g. \"2026-06-10T09:00:00Z\"), mapped to the ad set's `start_time`. When omitted the ad starts delivering immediately. For lifetime budgets Meta also requires `endDate`. Same field as on `POST /v1/ads/boost`.
     *
     * @return self
     */
     ...
     * Sets page_id
     *
     * @param string|null $page_id Meta only. The Facebook Page the ad runs as (`object_story_spec.page_id`). Defaults to the Page bound to the connection. Pass another Page ID to run the ad as that Page: any Page granted to the connection is accepted (for a business-login connection, every Page granted in Meta's dialog; list them with GET /v1/ads/instagram-accounts). The Instagram identity is re-resolved for that Page unless `instagramAccountId` is set. A Page the connection cannot see is a 400 on `pageId` naming the granted Pages.
     *
     * @return self
     */
     ...
     *
     * @return \Zernio\Model\CreateStandaloneAdRequestPlatformSpecificData|null
     */
     ...
     *
     * @param \Zernio\Model\CreateStandaloneAdRequestPlatformSpecificData|null $platform_specific_data platform_specific_data
     *
     * @return self
     */
     ...
     * Sets identity_id
     *
     * @param string|null $identity_id TikTok: the identity the ad runs as, from GET /v1/ads/tiktok-identities. Overrides the connected account's own identity; must be authorized on the advertiser.
     *
     * @return self
     */
