...
class PlatformAnalytics implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets error_message
     *
     * @param string|null $error_message Failure detail. On failed entries, why the post failed to publish. On unavailable entries, why analytics cannot be synced (e.g. Google Business Profile, a TikTok upload that never received a video id). On pending entries, the most recent analytics sync error for the account (null while no sync has failed), cleared after the next successful sync.
     *
     * @return self
     */
