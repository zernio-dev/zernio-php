...
class TargetingSpec implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets regions
     *
     * @param \Zernio\Model\CreateStandaloneAdRequestZipsInner[]|null $regions Region/state targeting. `key` is the platform location ID from /v1/ads/targeting/search?dimension=geo&geoType=region. An entry may also be that id as a plain string (`\"20321\"` is `{ key: \"20321\" }`).
     *
     * @return self
     */
     ...
     * Sets cities
     *
     * @param \Zernio\Model\TargetingSpecCitiesInner[]|null $cities City targeting. Optional `radius` + `distanceUnit` extend beyond the city limits; both must be set together or both omitted. `radius` is only honoured on platforms whose capability map allows city radius (Meta). An entry may also be the city id as a plain string (`\"1006410\"` is `{ key: \"1006410\" }`).
     *
     * @return self
     */
     ...
     * Sets zips
     *
     * @param \Zernio\Model\UpdateCampaignTargetingRequestTargetingLocationsOneOfRegionsInnerOneOf[]|null $zips Postal/ZIP targeting. `key` is the platform's postal location ID (e.g. Meta `US:94304`). Supported on Meta, Google, TikTok, Pinterest, X.
     *
     * @return self
     */
     ...
     *
     * @param string[]|null $audience_include Platform audience IDs to include, as returned by GET /v1/ads/audiences (Meta custom audience ids, TikTok audience ids, Pinterest customer list ids, LinkedIn segment ids (the platformAudienceId from GET /v1/ads/audiences; Zernio resolves it to the targetable LinkedIn ad segment, an unknown id returns 400), Google user list ids, X custom audience ids). Not supported on OpenAI (400).
     *
     * @return self
     */
