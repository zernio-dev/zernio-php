...
$since = new \DateTime('2013-10-20T19:20:30+01:00'); // \DateTime | Meta only. Earliest delivery date (YYYY-MM-DD). LinkedIn's archive does not filter by date, so it is a 400 there: filter on details.adStatistics.firstImpressionAt / latestImpressionAt instead (EU-delivered ads only).
$until = new \DateTime('2013-10-20T19:20:30+01:00'); // \DateTime | Meta only. Latest delivery date (YYYY-MM-DD); a 400 on LinkedIn, see since.
...
| **since** | **\DateTime**| Meta only. Earliest delivery date (YYYY-MM-DD). LinkedIn&#39;s archive does not filter by date, so it is a 400 there: filter on details.adStatistics.firstImpressionAt / latestImpressionAt instead (EU-delivered ads only). | [optional] |
| **until** | **\DateTime**| Meta only. Latest delivery date (YYYY-MM-DD); a 400 on LinkedIn, see since. | [optional] |
