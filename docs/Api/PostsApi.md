...
```php
createPost($create_post_request, $idempotency_key, $x_request_id): \Zernio\Model\CreatePost200Response
```
...
try {
    $result = $apiInstance->createPost($create_post_request, $idempotency_key, $x_request_id);
    print_r($result);
} catch (Exception $e) {
...
$post_id = 'post_id_example'; // string
$edit_post_request = {"platform":"facebook","content":"Updated post text with corrected information"}; // \Zernio\Model\EditPostRequest
